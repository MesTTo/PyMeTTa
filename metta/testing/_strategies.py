...
#: Where the census the fuzz lane draws from sits inside a checkout, generated
#: by `tests/conformance/petta_capture.py --census` and committed beside the
#: corpus it was read from. Relative to the workspace and joined to it only
#: when asked: an installed wheel has no workspace, `workspace()` raises by
#: design rather than guessing a tree, and resolving this at import scope took
#: metta.testing down with it for every install outside a project directory
#: [measured 2026-09-22]. `programs(census=...)` is the door for that case and
#: `_census_data` names it.
_CENSUS_IN_CHECKOUT = ("tests", "conformance", "petta", "HEADS.json")
...
def _census_data(census: dict | None) -> dict:
    ...
    if census is not None:
        return census
    try:
        found = workspace().joinpath(*_CENSUS_IN_CHECKOUT)
    except RuntimeError:
        # No checkout above this package, which is every ordinary install. The
        # refusal names the door rather than the missing path, because there is
        # no path to name and the caller's fix is to bring their own census.
        msg = (
            "programs() draws from a head census and this is an installed copy "
            "with no checkout above it, so there is none to read. Pass one: "
            "programs(census=json.loads(...)), or set METTA_WORKSPACE to a checkout."
        )
        raise FileNotFoundError(msg) from None
    if found.is_file():
        return json.loads(found.read_text(encoding="utf-8"))
    msg = (
        f"programs() draws from a head census and there is none at {found}. "
