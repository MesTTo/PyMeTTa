...
:- prolog_load_context(directory, Binding),
   absolute_file_name('../..', Dir,
                      [relative_to(Binding), file_type(directory)]),
   assertz(metta_py_dir(Dir)).
...
metta_py_bridge :- metta_py_dir(Dir),
                   py_add_lib_dir(Dir),
                   metta_py_opts(Opts),
                   py_call('metta._binding.host':stream_tag(), Tag, Opts),
...
metta_py_call(Call, Goal, Result) :-
    metta_py_bridge,
    metta_py_opts(Opts),
    metta_py_guard(Call, py_call('metta._binding.host':Goal, Raw, Opts)),
...
'py-iter'(Obj, Element) :-
    ...
    metta_py_guard(['py-iter', Obj],
                   ( py_iter('metta._binding.host':iterate(Obj), Raw, Opts),
...
'py-iter-once'(Obj, Element) :-
    ...
    metta_py_guard(['py-iter-once', Obj],
                   ( py_iter('metta._binding.host':iterate_once(Obj), Raw, Opts),
                     py_iter_item(Raw, Tag) )),
    metta_py_result(Raw, Element).
...
%
%So host.py ends a failed enumeration with the reserved pair instead, and
...
%because neither this file nor metta/_binding/shim.pl declares a module: both load into
...
py_iter_raise(Exception) :-
    py_call('metta._binding.host':stream_reraise(Exception), _),
...
%
%Every one of those is fixed in extensions/python/metta/_binding/surface.pl, which is the language's own
