...
from metta._atoms.factories import Atom, Expression, Grounded, Symbol, _decode, _encode, parse
from metta._catalog.declarations import Declaration, declared
...
from metta.vocabularies import WIRE_TAGS, WireClass, WirePayload
...
def head_entries(space: _root.Space | Any) -> list[dict[str, Any]]:
...
def openapi_document(
    spaces: _collections_abc.Mapping[str, _root.Space | Any],
    *,
    secured: bool = False,
) -> dict[str, Any]:
...
def _catalog(spaces: _collections_abc.Mapping[str, _root.Space | Any]) -> _Catalog:
...
def graphql_sdl(spaces: _collections_abc.Mapping[str, _root.Space | Any]) -> str:
...
def execute_graphql(
    schema: Any,
    root: _collections_abc.Mapping[str, Any],
    request: _collections_abc.Mapping[str, Any],
) -> dict[str, Any]:
    """Run one GraphQL request against a schema and answer the response body."""
    return _executor().execute(schema, root, request)


def graphql_heads(spaces: _collections_abc.Mapping[str, _root.Space | Any]) -> tuple[_Head, ...]:
