...
Guarantees:
  ...
  - ``format(atom, spec)`` answers ``str(atom)`` for an empty spec, the
    library's own rendering for a spec of ``metta._atoms.templates``'s table, and
...
if TYPE_CHECKING:
    import metta._binding.positions
    import metta._binding.runtime
    import metta._spaces.ambient
    import metta._spaces.results  # noqa: F401 -- child of the deferred package namespace
if TYPE_CHECKING:
    from metta import _binding
else:
    _binding = lazy('metta._binding')
if TYPE_CHECKING:
    from metta import _spaces
else:
    _spaces = lazy('metta._spaces')
...
class Atom:
    ...
    def __repr__(self) -> str:
        from metta._atoms.factories import _pretty  # noqa: PLC0415  -- atoms owns layout
    ...
    def vars(self) -> tuple[Variable, ...]:
        ...
        """
        from metta._atoms.factories import _variables  # noqa: PLC0415  -- atoms owns tree traversal
    ...
    def map(self, transform: Callable[[Atom], Atom]) -> Atom:
        ...
        from metta._atoms.factories import _map_atoms  # noqa: PLC0415  -- atoms owns tree traversal
    ...
    def alpha_eq(self, other: Atom) -> bool:
        ...
        from metta._atoms.factories import _alpha_eq  # noqa: PLC0415  -- atoms owns equivalence
    ...
    def unify(self, other: Atom, *more: Atom) -> Mapping[Atom, Atom] | None:
        ...
        """
        from metta._atoms.factories import unify  # noqa: PLC0415  -- atoms owns unification
    ...
    def subs(self, bindings: Mapping[Atom, Any] | Any) -> Atom:
        ...
        """
        from metta._atoms.factories import _to_atom  # noqa: PLC0415  -- atoms owns encoding

        if isinstance(bindings, _spaces.results.Row):
            bindings = {
    ...
    def cast(self, type_: Any, /) -> Any:
        ...
        return _spaces.ambient._ambient_space().cast(self, type_)
    ...
    def __format__(self, spec: str) -> str:
        ...
        from metta._atoms.templates import (  # noqa: PLC0415 -- _templates reads this module
            formatted,
        )
...
class Symbol(Atom):
    ...
    def origin(self) -> tuple[Any, ...]:
        ...
        """
        return _binding.positions.head_origins(_spaces.ambient._ambient_space(), self.name)
...
class Grounded(Atom):
    ...
    def __lt__(self, other: Any) -> bool:
        if isinstance(other, Atom):
            from metta._atoms.factories import order_key  # noqa: PLC0415  -- atoms owns the order
...
def _apply_operator_lowering(
    ...
    *,
    ...
    form: Any = (
        ...
        # policy-inventory-exempt: mechanism-internal; reason=symbol and provided are the two lowering-table kinds whose form is a MeTTa head to apply to the operands; evidence=extensions/python/metta/_atoms/operators.py:OperatorLowering
...
def _standard_order_lt(self: Atom, other: Any) -> bool:
    ...
    if not isinstance(other, Atom):
        raise _atom_plain_order_error(self, other, "<")
    from metta._atoms.factories import order_key  # noqa: PLC0415  -- atoms owns the public order
