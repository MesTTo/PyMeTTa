...
#: it improbable. The shape follows ``metta._atoms.factories.fresh``'s ``__metta_fresh_``,
#: the library's existing generated-name convention.
if TYPE_CHECKING:
    import metta._binding.json  # noqa: F401 -- child of the deferred package namespace
if TYPE_CHECKING:
...
_CONVERSIONS: dict[str, _collections_abc.Callable[[Any], str]] = {"r": repr, "s": str, "a": ascii}
...
def read_source(
    source: Any,
    values: _collections_abc.Mapping[str, Any],
    *,
    called: str,
    reserved: tuple[str, ...] = (),
...
class _Holes:
    ...
    def prefix(self, segment: str, interpolation: _atoms_designation.InterpolationLike) -> str:
    ...
    def check_unused(
        self, values: _collections_abc.Mapping[str, Any], implicit: frozenset[str] = frozenset()
    ) -> None:
...
def _template_parts(
    template: _atoms_designation.TemplateLike, sink: _Holes, seen: tuple[int, ...]
) -> None:
...
def _debug_fold(
    segment: str, interpolation: _atoms_designation.InterpolationLike
) -> tuple[str | None, str]:
...
def _keyword_parts(
    text: str, values: _collections_abc.Mapping[str, Any], sink: _Holes
) -> None:
...
def _field_value(field: str, values: _collections_abc.Mapping[str, Any], sink: _Holes) -> Any:
...
def _atom(value: Any, conversion: str | None, spec: str, shown: str) -> Atom:
    ...
    if spec in ("sym", "expr"):
        ...
        if spec == "sym":
            return Symbol(value)
        # Deferred because metta._atoms.factories carries this door's own face, so
        # importing it here at module scope would be an import cycle.
        from metta._atoms.factories import parse  # noqa: PLC0415
...
def _json_text(value: Any) -> str:
    ...
    """
    return _binding.json.dumps(_json_value(value)).decode("utf-8")
...
def apply(atom: Atom, holes: _collections_abc.Mapping[str, Atom]) -> Atom:
