...
Guarantees:
  - order_key matches the engine's msort across every public atom kind,
    ...
    test_native_rational_sorts_before_an_opaque_numeric_subclass; commit=WORKTREE]
  ...
  - public atom classes retain the metta._atoms.factories pickle path after internal
...
import metta._atoms.wire as _wire
from metta._atoms.designation import InterpolationLike, TemplateLike
...
from metta._atoms.model import encode as _encode
...
def parse(source: str | TemplateLike, /, **values: Any) -> Atom:
    ...
    if values or not isinstance(source, str):
        ...
        templates = importlib.import_module("metta._atoms.templates")
...
def _read_one(source: str) -> Atom:
    ...
    engine = lazy('metta._binding.runtime')
