...
from metta._atoms.factories import Atom, Expression, Symbol, Variable, _atom_from_wire, substitute
...
class Written(NamedTuple):
    ...
    atom: Atom
    origin: _binding.positions.Origin | None = None
    registers: tuple[str, ...] = ()
...
class Declaration:
    ...
    origin: _binding.positions.Origin | None = None
    documentation_origin: _binding.positions.Origin | None = None
...
def written(subject: _root.Space | Any) -> _collections_abc.Iterator[Written]:
    ...
    rows: _collections_abc.Iterable[Written] = source()
    return iter(rows)


def declarations(subject: _root.Space | Any) -> tuple[Declaration, ...]:
...
def declarations_in(items: _collections_abc.Iterable[Atom | Written]) -> tuple[Declaration, ...]:
    ...
    def rows_for(name: str, origin: _binding.positions.Origin | None) -> _Rows:
...
def declared(space: _root.Space | Any) -> tuple[Declaration, ...]:
...
def _documentation_rows(space: _root.Space | Any) -> dict[str, Expression]:
...
def inferred(space: _root.Space | Any) -> tuple[Inference, ...]:
