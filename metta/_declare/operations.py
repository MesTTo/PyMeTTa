...
from metta._atoms.designation import _DEFAULT_SPACE, _P, _R, _OperationName, _SpaceId
...
from metta._catalog.annotations import callable_name as _callable_name
...
def class_declarations(cls: type) -> list[Expression]:
    ...
    """  # noqa: D205  -- the API contract is one continuous invariant, not summary-and-body prose
    return list(lazy('metta.convert').declarations(cls))
...
def _operation_kind(fn: Callable, transport: Transport) -> str:
    if transport not in typing.get_args(Transport):
        msg = f"transport must be one of {typing.get_args(Transport)}, got {transport!r}"
...
def _engine_positions(params: list[inspect.Parameter], fn: Callable) -> list[int]:
    ...
    """  # noqa: D205  -- the API contract is one continuous invariant, not summary-and-body prose
    metta_type = lazy('metta._faces.metta').MeTTa
    try:
...
def _with_engine(runtime: Any, fn: Callable, positions: list[int]) -> Callable:
    ...
    @functools.wraps(fn)
    def woven(*args):
        space_api = _importlib.import_module('metta._spaces.handle')
        ...
        # commit=39092863ae34184a9f955f185ff57c1ff177ec40].
        engine = lazy('metta._faces.metta').MeTTa(
...
def register[**P, R](
    ...
    *,
    ...
    if kind == "async":
        ...
        parallel_api = lazy('metta.parallel')
        parallel_api._ensure_thread_library(
            lazy('metta._faces.space').Space(space, _runtime=runtime)
        )
    ...
    # this module feeds.
    from metta._declare.rules import (  # noqa: PLC0415 -- operation callbacks run after declaration initialization
...
@_doors.door(
    ...
    alias='op',
    async_signature=_doors.Signature("self, fn: Callable, /, *, effect: Any, name: Any = None, transport: Any = 'encoded', declarations: Any = (), arities: Any = None, inverse: Any = None", returns='Any'),
...
def _classified(
    space: _root.Space, fn: Callable | None, effect: EffectClass, options: dict[str, Any]
) -> Any:
    ...
    """
    if fn is None:
        return space.op(effect=effect, **options)
    return space.op(fn, effect=effect, **options)
...
@_doors.door(
    ...
    alias='pure',
    async_signature=_doors.Signature('self, fn: Callable, /, **options: Any', returns='Any'),
    async_reason="the sync method's fn=None form returns a DECORATOR, and a decorator handed back across the worker would register on the caller's thread rather than the engine's. Only the applied form crosses",
    sugar_of=_doors.Sugar('space:op', (('effect', 'pureStructural'),)),
)
def pure(space: _root.Space, fn: Callable | None = None, /, **options: Any) -> Any:
    ...
    """
    return _classified(space, fn, EffectClass.pureStructural, options)
...
@_doors.door(
    ...
    async_signature=_doors.Signature('self, fn: Callable, /, **options: Any', returns='Any'),
    async_reason="the sync method's fn=None form returns a DECORATOR, and a decorator handed back across the worker would register on the caller's thread rather than the engine's. Only the applied form crosses",
    sugar_of=_doors.Sugar('space:op', (('effect', 'readOnlyLookup'),)),
)
def reads(space: _root.Space, fn: Callable | None = None, /, **options: Any) -> Any:
    ...
    the mechanism and this line shows the surface.
    """
    return _classified(space, fn, EffectClass.readOnlyLookup, options)
...
@_doors.door(
    ...
    async_signature=_doors.Signature('self, fn: Callable, /, **options: Any', returns='Any'),
    async_reason="the sync method's fn=None form returns a DECORATOR, and a decorator handed back across the worker would register on the caller's thread rather than the engine's. Only the applied form crosses",
    sugar_of=_doors.Sugar('space:op', (('effect', 'writesState'),)),
)
def writes(space: _root.Space, fn: Callable | None = None, /, **options: Any) -> Any:
    ...
    the mechanism and this line shows the surface.
    """
    return _classified(space, fn, EffectClass.writesState, options)
...
@_doors.door(
    ...
    async_signature=_doors.Signature('self, fn: Callable, /, **options: Any', returns='Any'),
    async_reason="the sync method's fn=None form returns a DECORATOR, and a decorator handed back across the worker would register on the caller's thread rather than the engine's. Only the applied form crosses",
    sugar_of=_doors.Sugar('space:op', (('effect', 'oracleIO'),)),
)
def io(space: _root.Space, fn: Callable | None = None, /, **options: Any) -> Any:
    ...
    the mechanism and this line shows the surface.
    """
    return _classified(space, fn, EffectClass.oracleIO, options)
...
def unregister_op(space: _root.Space, name: str) -> None:
