...
# policy-inventory-exempt: mechanism-internal; reason=these five names are the lowering table's own entry kinds, read only by the apply and method doors that walk the table; evidence=bindings/python/metta/_operator_lowerings.py:OperatorLowering
...
class OperatorLowering(NamedTuple):
    ...
    # policy-inventory-exempt: mechanism-internal; reason=Python's operator protocol has only unary and binary dunders, so this is the table entry's own arity field; evidence=bindings/python/metta/_operator_lowerings.py:OperatorLowering
    arity: Literal[1, 2] = 2
