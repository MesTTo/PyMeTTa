...
def _runtime_for_name(space: str) -> Any:
    ...
    if runtime is not None:
        return runtime
    # runtime imports intent recording during initialization
    from metta._binding.runtime import runtime as current_runtime  # noqa: PLC0415
...
def external_caller(frame: FrameType | None = None) -> FrameType | None:
    """Find the first external frame, starting at a supplied caller if any."""
...
class _LoopCrossings(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if (
            ...
            # policy-inventory-exempt: mechanism-internal; reason=map and filter are the two eager Python builtins that repeatedly invoke their first argument while consuming the iterable; evidence=extensions/python/metta/_spaces/intents.py:_LoopCrossings.visit_Call
