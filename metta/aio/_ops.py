...
from metta._errors.errors import NotReducible
...
def prepare(name: str, tagged_args: list, parent_context: Any) -> int:
    ...
    import metta._binding.dispatch as _ops  # noqa: PLC0415 -- callback dispatch follows worker initialization
    import metta._spaces.handle as _space  # noqa: PLC0415  -- callbacks import satellites lazily
...
def _completed(token: int, pending: _Pending, task: asyncio.Task[Any]) -> None:
    import metta._binding.dispatch as _ops  # noqa: PLC0415  -- completion uses the registry codec
