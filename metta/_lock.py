...
Assumes:
  ...
  - `metta_py_source_loads/2` answers inside one transaction and refuses while
    a load is in flight [source: extensions/python/metta/shim.pl]
Guarantees:
...
from .errors import LockDrift, MettaError
...
def take(rt: Any) -> Lock:
    ...
    """
    from .library import roster  # noqa: PLC0415  -- a satellite, deferred like the rest
...
def _engine_table(rt: Any) -> dict[str, str]:
    ...
    """
    from ._engine import (  # noqa: PLC0415  -- the janus bridge, as info() reaches it
        _resolve_metta_path,
        bridge,
    )
...
def _library_digest(rt: Any, files: Iterable[Path]) -> str:
