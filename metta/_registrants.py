...
def _pyarrow_claims() -> Any:
    """pyarrow, when it is installed; nothing otherwise.

    pyarrow writes and reads the IPC streaming format and is its reference
    implementation; the capsule doors above need only nanoarrow.
    """
    return optional_module("pyarrow")


def _pyarrow_types(pa: Any) -> dict[str, Any]:
    from ._arrow import BOOL, FLOAT64, INT64, TEXT, UTF8  # noqa: PLC0415  -- the five column kinds
...
def _pyarrow_schema(names: Any, kinds: Any, declared: Any) -> Any:
    ...
    """
    from ._arrow import TEXT  # noqa: PLC0415  -- the undeclared column kind

    pa = _pyarrow_claims()
    types = _pyarrow_types(pa)
    fields = []
...
def _pyarrow_stream(schema: Any, columns: Any) -> bytes:
...
def _pyarrow_read(raw: bytes) -> Any:
...
def _pyarrow_concat(tables: Any) -> Any:
    """The drained chunks of one cursor as one table."""
    return _pyarrow_claims().concat_tables(list(tables))


seam.ipc.register(
    "pyarrow",
    source="shipped",
    claims=_pyarrow_claims,
    schema=_pyarrow_schema,
    stream=_pyarrow_stream,
    read=_pyarrow_read,
    concat=_pyarrow_concat,
    missing=_IPC_MISSING,
)
