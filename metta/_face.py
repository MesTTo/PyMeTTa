"""Purpose: write a module's own signatures out as a MeTTa face, so a library
that wraps a Python module is generated from that module rather than kept by
hand: one arrow per reachable call form, the `(@doc ...)` atom the docstring
already carries, the equation that applies the name through `py-call`, and an
effect class derived from what the signature says it answers.
...
from ._documentation import documentation_atom, first_paragraph
from ._name_mapping import attribute_name
from ._type_annotations import metta_type_for, resolved_annotations
from .atoms import Expression, Symbol, Variable
...
#: [source: extensions/python/metta/library.py:_FIELD].
...
class Face:
    ...
    def _effect(self, name: Name, forms: Mapping[int, CallForm]) -> str:
        ...
        [source: extensions/python/metta/arrays.py, its Guarantees;
        extensions/python/bridge.pl, seam:extension_builtin('py-call', oracleIO)].
        """
...
def read(text: str) -> Manifest | None:
    ...
    for line in text.splitlines():
        ...
        if field is None:
            ...
            #[source: extensions/python/metta/library.py:_summary].
            if describing:
...
def _module_of(path: str) -> Any:
    ...
    [source: extensions/python/metta_py.py, resolve].
    """
...
def _module_names(module: Any) -> list[str]:
    ...
    [source: extensions/python/metta/integrate.py:_module_callable_names].
...
def _selected(statement: Import) -> Iterator[Name]:
...
def _signature_of(target: Any) -> inspect.Signature | None:
    ...
    [source: extensions/python/metta/_type_annotations.py:resolved_annotations].
    """
...
def _derived_effect(forms: Mapping[int, CallForm]) -> str:
...
def _equation(name: Name, form: CallForm) -> str:
    ...
    [source: extensions/python/bridge.pl, 'py-call'/3].
    """
...
def _documentation(name: Name, forms: Mapping[int, CallForm]) -> str | None:
