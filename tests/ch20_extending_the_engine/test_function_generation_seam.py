...
Guarantees:
  - ``metta_py_function_generation/1`` is monotonic across a public function
...
def test_generation_tracks_definitions_but_not_evaluation(metta):
    ...
    before = metta.runtime.once("metta_py_function_generation(G)")["G"]
    metta.run("(= (p14-python-generation $x) $x)")
    defined = metta.runtime.once("metta_py_function_generation(G)")["G"]
    metta.eval("(+ 1 2)")
    evaluated = metta.runtime.once("metta_py_function_generation(G)")["G"]
