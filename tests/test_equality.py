...
def test_mixed_numeric_equality_answers_what_the_arbiter_answers(declared):
    """Match LeaTTa's mixed numeric equivalence.

    `Ground.equiv` promotes the integer with `Float.ofInt` in both mixed numeric
    cases (`MettaHyperonFull/Core/Atom.lean:47-62`), and `Atom.equiv` uses that
    relation for grounded atoms (lines 110-116).
    """
