"""Purpose: the three acceptance criteria of the metatheory cluster, each
    ...
    everywhere below. Critical pairs are a rewriting notion; a translator rule
    is applied by calling its compiled clause, so the rule and the term unify,
    and that is narrowing. The termination reports therefore come from
    engine/narrowing.pl's reduction rather than from engine/trs.pl's order
    directly. The confluence checker
...
def test_each_narrowing_citation_in_the_plan_states_which_it_covers(repo_root):
    ...
    correction: the repository cites only what it BUILT ON, which is nine
    files. So this pins those, on both sides. Every one of them says which
