"""Purpose: the face generator end to end, over a fixture module carrying one
of every shape it has a rule for: the head spelling, the call forms a signature
reaches, the types, the documentation, the three `py-call` lowerings, the
effect classes, the header that is read back as the face's input, and the
refusals a name the module cannot describe earns.
