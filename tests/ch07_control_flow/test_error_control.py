...
Guarantees:
  ...
  [tested: test_switch_reads_a_key_with_no_answers_as_no_answer; commit=f88aa8be03cb64cb59d3307515ded8701f418321]
...
# Upstream PeTTa at ae66fa8e is the arbiter; the pins below come from an
# earlier reference corpus's case-and-switch program, recorded against the
# pinned Hyperon 0.2.10 binary, whose measured block carries the five lines
# asserted below [assumed: not re-measured against upstream PeTTa]. case is
...
def test_switch_reads_a_key_with_no_answers_as_no_answer() -> None:
    ...
    metta = MeTTa().space("&errorcontrol-switch")
    metta.run(_KEY)
...
def test_a_thrown_reason_travels_as_a_produced_error() -> None:
    ...
    metta = MeTTa().space("&errorcontrol-throw")
