...
def test_setting_declaration_reaches_every_projection(metta, monkeypatch):
    ...
    class FlaggedConfig(bounds.Config):
        layout_flag = bounds.Setting(
            5, "A flag declared by this fixture.", environment="METTA_LAYOUT_FLAG",
            startup=True, flag="layout_flag",
        )

    header = boundsgen.c_header(FlaggedConfig)
    assert '#define MT_LAYOUT_FLAG_FLAG "layout_flag"' in header
