...
def test_inline_source_becomes_a_metta_function(space):
    ...
    assert space.value("(rp-square 7)") == 49
...
def test_a_file_of_prolog_becomes_metta_functions(space, tmp_path):
    ...
    assert space.value("(rp-triple 14)") == 42
    assert space.value("(rp-negate 5)") == -5
...
def test_a_special_form_name_is_refused(space):
    ...
    assert space.value("(if True 1 2)") == 1
...
def test_generated_sources_do_not_erase_each_other(space):
    for i in range(4):
        ...
        del generated
    assert [space.value(f"(rp-gen{i} 10)") for i in range(4)] == [10, 11, 12, 13]
...
def test_a_python_operation_is_not_silently_replaced(space):
    ...
    with pytest.raises(EngineError, match="another extension tier"):
        space.register_prolog("'rp-owned'(X, R) :- R = [prolog, X].",
                              names=["rp-owned"])
    assert str(space.value("(rp-owned 1)")) == '("python" 1)'
...
def test_a_prolog_registration_is_not_silently_replaced(space):
    ...
    assert str(space.value("(rp-mine 1)")) == "(prolog 1)"
...
def test_a_rival_source_is_refused_before_it_can_clobber(space, tmp_path):
    ...
    assert space.value("(rp-rival-norm 1)") == 20
...
def test_a_rival_declaring_source_is_refused_before_it_can_clobber(space, tmp_path):
    ...
    assert space.value("(rp-declared-norm 1)") == 20
...
def test_prolog_registration_is_cheaper_than_python_registration(space):
    ...
    for which in ("fast", "slow"):
        space.value(f"(rp-drive {which} 10)")
        with space.stats() as counted:
            space.value(f"(rp-drive {which} {calls})")
        costs[which] = counted.inferences / calls

    assert space.value("(rp-fast 41)") == space.value("(rp-slow 41)") == 42
...
"""
    try:
        ...
        assert space.value("(rp-inline-scale 3)") == 21
    finally:
        with contextlib.suppress(PettaError):
...
def test_a_file_declares_its_own_exports(space, declared):
    ...
    assert space.value("(rp-demo-scale 3)") == 30
    ...
    assert str(space.value("(rp-demo-shape (+ 1 2))")) == "(shape (+ 1 2))"
...
def test_an_extension_unloads_whole(space, declared):
    ...
    assert str(space.value("(rp-demo-scale 3)")) == "(rp-demo-scale 3)"
...
def test_a_declaration_without_an_extension_still_reports_its_names(space, tmp_path):
    ...
    assert space.value("(rp-bare-scale 4)") == 40
...
def test_a_provider_only_file_registers_no_functions_and_is_accepted(space, tmp_path):
    ...
    try:
        ...
        assert str(space.value("(collapse (get-atoms &rp-provider-demo))")) == "((fact a))"
    finally:
        with contextlib.suppress(PettaError):
...
def test_a_file_that_declares_nothing_does_not_load_either(space, tmp_path):
    ...
    assert str(space.value("(collapse (get-atoms &rp-silent-demo))")) == "()"
...
def test_two_libraries_exporting_one_name_can_both_be_registered(space, rival_modules):
    ...
    assert space.value("(rp-liba-norm -5)") == 5
    assert space.value("(rp-libb-norm -5)") == 25
...
def test_a_declared_det_function_answers_normally(space, det_library):
    ...
    assert space.value("(rp-det-clean 1)") == 2
...
def test_a_compiled_library_registers_from_python(space):
    ...
    assert space.value("(c-bump 41)") == 42
...
def test_an_opaque_handle_crosses_as_an_ordinary_value(space):
    ...
    assert space.value("(vector-length (vector-new 1000))") == 1000
    assert space.value("(vector-nth (vector-new 1000) 700)") == 700
