...
- `conformance/` holds the arbiters and oracles that decide what MeTTa MEANS.
  The vendored upstream PeTTa corpus, the critical-pair enumerator, the
  differential against the CLI. A finding here says this engine disagrees with
  an authority, which is a different claim from a broken feature.
