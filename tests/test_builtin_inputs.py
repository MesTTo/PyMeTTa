...
def test_arithmetic_inverts_past_the_linear_case_or_refuses_with_the_reason():
    ...
    # The fragment, one unknown among integers, forwards and backwards.
    engine.run("(= (double $x) (* 2 $x))")
    assert answers("!(double 5)") == ["10"]
    assert answers("!(let 10 (double $x) $x)")[0] == "5"
    ...
    assert answers("!(collapse (let 7 (double $x) $x))") == ["()"]
