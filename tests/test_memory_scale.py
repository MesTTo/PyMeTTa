...
def test_curve_fit_distinguishes_linear_and_quadratic_growth():
    """Classify exact synthetic linear and quadratic curves."""
...
def test_curve_fit_recognises_a_bounded_linear_cache():
    """Recognize linear growth followed by a fixed cache cap."""
...
def test_aggregation_accepts_controlled_instruction_samples():
    """Promote controlled instructions as a case's primary metric."""
...
def test_memory_scale_cli_runs_fresh_workers(tmp_path):
    """Run each quick-lane point in its own spawned process."""
...
def test_stream_curve_excludes_wire_cache_growth(tmp_path):
    """Keep the streaming curve independent of wire-cache growth."""
...
def test_memory_scale_cli_gates_object_reclamation(tmp_path):
    """Gate object and box reclamation through the CLI worker path."""
