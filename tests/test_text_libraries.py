...
@given(TEXT)
def test_a_full_slice_is_the_original(text_space, text):
...
@settings(max_examples=40, deadline=None)
@given(TEXT)
def test_chars_round_trip(text_space, text):
...
def test_repeat_multiplies_the_length(text_space, text, times):
...
def test_padding_reaches_the_width_and_never_shortens(text_space, text, width):
...
@settings(max_examples=40, deadline=None)
@given(TEXT)
def test_trim_is_idempotent(text_space, text):
...
@given(TEXT, TEXT)
def test_replacing_a_string_with_itself_changes_nothing(text_space, text, part):
...
def test_json_round_trips_scalars(text_space, value):
...
def test_json_round_trips_arrays(text_space, numbers):
