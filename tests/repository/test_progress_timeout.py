...
pytest with pytest-timeout and that plugin only, and show both halves of the
contract: an item slowed by a competitor for its CPU is not stopped for it,
and an item that blocks still is.
Guarantees:
...
def _child(directory, source, budget, *plugins, cpu=None):
    """Run one test file under a `budget`-second timeout in a child pytest
    that loads only pytest-timeout and the plugins named.
