...
def test_a_space_accessor_costs_no_engine_call_outside_a_callback():
    """Reading a handle's name asks the host, never the engine, for its scope."""
    with metta.MeTTa() as m:
        space = m.self
        assert space.name
        with m.stats() as empty:
            pass
        with m.stats() as reads:
            for _ in range(500):
                assert space.name
        # The block's own bookkeeping is the whole cost; the reads add nothing.
        assert reads.inferences == empty.inferences


def test_the_engine_scope_is_read_inside_a_callback():
    """An operation called from a `(scope ...)` body sees the engine's scope."""
    seen: list[str | None] = []
    with metta.MeTTa() as m:
        m.run("!(import! &self (library lib_thread))")

        @m.op(name="sc-probe", effect="oracleIO")
        def probe() -> bool:
            seen.append(scope_context.current())
            return True
