...
Guarantees:
  - ``metta_py_limited/6`` restores the calling thread's prior stack limit on
...
def test_janus_stack_scope_restores_on_all_exits(metta):
    ...
    success = runtime.once(
        ...
        "metta_py_limited(-1, -1, Scoped, metta_py_run, "
        "['!(+ 1 2)', '&self'], Out), "
    ...
    failure = runtime.once(
        "current_prolog_flag(stack_limit, Before), "
        "Scoped is Before + 1048576, "
        "catch(metta_py_limited(-1, -1, Scoped, metta_py_run, "
