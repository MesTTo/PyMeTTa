...
compiler to say why, measured. This does the same for `Space`'s public doors:
it classes each one, and requires every door that is macro-expressible ON THE
PUBLIC SURFACE to carry a reason for existing.

The classification is DERIVED FROM THE CODE, not maintained by hand. A door
that starts reaching another public door becomes derived here on its next run
and fails the gate until its row says what it buys.

Assumes:
  - a door whose body calls another public method of the same class is
    macro-expressible by that door, which is Felleisen's test for a form that
    is not primitive: it can be rewritten locally, without restructuring
  - the four other classes are structural rather than shrinkable. A door that
...
Guarantees:
  - every derived door has exactly one row, and every row names a live door,
    so the ledger cannot drift from the class [tested:
    test_the_shrink_ledger_covers_every_derived_door]
  - a row says what the door BUYS, because a derived door with no answer is
    the shrink target and the ledger exists to name those
    [tested: test_every_ledger_row_states_what_its_door_buys]
Fails when: read as a count. The number of derived doors is not a score; a
...
def classify(cls: type) -> dict[str, tuple[str, tuple[str, ...]]]:
    """Every public door, its class, and what its body reaches."""
...
def findings(rows: dict[str, tuple[str, tuple[str, ...]]]) -> list[str]:
    ...
    for name in sorted(derived - set(DERIVED)):
        ...
        out.append(
            ...
            f"tools/ledger_entries.py, or shrink the door."
        )
    out.extend(
        f"{name}: has a ledger row but no longer reaches another public "
        f"door. Remove the row."
...
def page(rows: dict[str, tuple[str, tuple[str, ...]]]) -> str:
    ...
    lines = [
        ...
        "maintained by hand.",
        "",
        f"`Space` publishes **{len(rows)}** doors.",
        "",
        "| class | count | what it means |",
        "|---|---|---|",
        f"| primitive | {counts['primitive']} | speaks to the engine; nothing on this surface expresses it |",
        f"| derived | {counts['derived']} | its body reaches another PUBLIC door, so it is macro-expressible by one |",
...
def main(argv: list[str]) -> int:
    ...
    for problem in problems:
        print(f"  {problem}")
    print(f"{len(problems)} finding(s) over {len(rows)} doors")
