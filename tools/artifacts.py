...
#: The two files this tool writes into, named ONCE. Each was written out at
...
ARTIFACTS = (
    Artifact(
        ...
        (Output(CHECK_SH, (BEGIN, END)), Output(CHECK_SH, (LANES_BEGIN, LANES_END)),
         Output(GUIDE_MD, (DOC_BEGIN, DOC_END))),
...
def projections(root: Path = ROOT, records: tuple[Artifact, ...] = ARTIFACTS) -> dict[Path, str]:
    ...
    check, guide = root / CHECK_SH, root / GUIDE_MD
