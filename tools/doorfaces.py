...
def header(source: str, path: str) -> str:
    """Name the emitter, its inputs and the drift lane on each generated face.

    All three are read from the artifact manifest for the output at `path`.
    """
    return ('"""Purpose: expose the generated ' + source + ' door face.\n\n'
            + textwrap.fill(notice(path), width=78)
...
def synchronous(rows: Iterable[Door], root: Path) -> dict[Path, str]:
    ...
    for tier, class_name, base, module in [
        ...
        code = header(class_name, 'extensions/python/' + module.replace('.', '/') + '.py') + emitter.imports_text()
...
def asynchronous(rows: Iterable[Door], root: Path) -> dict[Path, str]:
    ...
    code = header('AsyncMeTTa', 'extensions/python/metta/aio/_mirror.py') + emitter.imports_text()
