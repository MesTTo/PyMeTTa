...
Guarantees: each Rows and Answers extension door row is declared twice, for the
  type checker and as the runtime's map from door name to base door, and never
  with the arguments it fixes [tested 2026-09-26T00:24:42+10:00:
...
def projections(rows: tuple[Door, ...]) -> dict[Path, str]:
    ...
    for owner in (Owner.rows, Owner.answers):
        ...
        aliases = [f'        {row.python} = _door_types._{owner.value}_{row.python}' for row in declared]
        # The same declarations for the runtime: each door's name and the core
        # door it is sugar over, never the arguments it fixes, which name the
        # package's library.
        bases = ', '.join(f'{row.python!r}: {row.sugar_of.base!r}' for row in declared)
        content = [start, '    # Generated by tools/doorgen.py from extension door marks.', '    if TYPE_CHECKING:', *(aliases or ['        pass']),
                   f'    _extension_doors: ClassVar[dict[str, str]] = {{{bases}}}', end]
