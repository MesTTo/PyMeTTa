...
Assumes:
  ...
  - SWI statistics/2 and predicate_property/2 report exact structural
    counters for the process embedded by metta.
Guarantees:
...
def _swi_snapshot(space: Space) -> dict[str, int]:
...
def _space_module_snapshot(space: Space) -> dict[str, int]:
    row = space.runtime.once(
...
def _snapshot(space: Space) -> dict[str, int]:
...
def _drop_all(spaces: Sequence[Space]) -> None:
    for space in spaces:
        space.drop()
...
def _join_width(size: int, *, projection: bool, mork: bool = False) -> dict[str, int]:
    root = MeTTa()
    space = root.space("&mork:memscale-join") if mork else root.space()
    facts = []
    patterns = []
...
def _compiled_predicate_bytes(space: Space) -> dict[str, int]:
    row = space.runtime.once(
...
def _sum_live_module_bytes(spaces: Sequence[Space]) -> int:
    total = 0
    for space in spaces:
...
def _support_edge_count(root: Space) -> int:
...
def _execution_owned_count(root: Space, name: str) -> int:
...
def _space_reuse(size: int) -> dict[str, int]:
    ...
    def cycle() -> tuple[list[str], int]:
        spaces = [root._new_space() for _ in range(size)]
        names = [str(space.name) for space in spaces]
...
def _table_bytes(space: Space) -> dict[str, int]:
    row = space.runtime.once(
...
def _table_reclamation(size: int) -> dict[str, int]:
    ...
    space.run(
        f"(= (ms-table $x) (match {space.name} (ms-table-row $x) $x))"
    )
    ...
    try:
        return _measure(
            ...
            extras=lambda result: {
                ...
                "stored_count": len(target) if target is not None else int(result),
            },
        )
    finally:
        source.drop()
